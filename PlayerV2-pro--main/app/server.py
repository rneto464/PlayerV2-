...
def setup_application():
    ...
    try:
        ...
        service_youtube = YouTubeMusicService()  
...
def get_local_playlist_tracks(playlist_id):
    ...
    try:
        cursor = conn.cursor()
        if not cursor.execute("SELECT id FROM playlists_salvas WHERE id = ? AND usuario_id = ?", (playlist_id, session['internal_user_id'])).fetchone(): return jsonify({"error": "Não encontrado."}), 404
        musicas = [{'spotify_id': r[0], 'titulo': r[1], 'artista': r[2], 'preview_url': r[3], 'artista_id': r[4], 'service_name': r[5]} for r in cursor.execute("SELECT musica_id, titulo_musica, artista_musica, preview_url_musica, artista_id, service_name FROM playlist_musicas WHERE playlist_id = ?", (playlist_id,))]
...
def create_local_playlist():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("INSERT OR IGNORE INTO playlists_salvas (usuario_id, nome_playlist, service_name) VALUES (?, ?, ?)", (session['internal_user_id'], playlist_name, active_service_name))
...
def rename_playlist_api(playlist_id):
    ...
    try:
        cursor = conn.cursor()
        if cursor.execute("SELECT id FROM playlists_salvas WHERE nome_playlist = ? AND usuario_id = ?", (new_name, session['internal_user_id'])).fetchone(): return jsonify({"error": "Nome já existe."}), 409
...
@app.route('/api/test_tags', methods=['POST'])
def test_tags_api():
    if 'internal_user_id' not in session: return jsonify({"error": "Utilizador não autenticado."}), 401
    engine = app_context['engine']; engine.music_service = _get_active_service()
    if not engine.music_service: return jsonify({"error": "Serviço de música não encontrado."}), 500
    data = request.get_json(); tags = data.get('tags')
    if not tags: return jsonify({"error": "Nenhuma tag fornecida."}), 400
    print("Tags recebidas:", tags)
    print("Serviço ativo:", type(self.music_service))
    result = self.music_service.search_tracks(" ".join(tags))
    print("Resultado yt-dlp:", result)
